...
const GPU_DATA = [
  ...
  { id: "rx7800xt", name: "AMD Radeon RX 7800 XT 16 GB", raster: 105, rt: 68, vram: 16, brand: "amd", gen: 7000, frameGen: false },
  { id: "rx7900xt", name: "AMD Radeon RX 7900 XT 20 GB", raster: 145, rt: 94, vram: 20, brand: "amd", gen: 7000, frameGen: false },
  { id: "rx7900xtx", name: "AMD Radeon RX 7900 XTX 24 GB", raster: 165, rt: 108, vram: 24, brand: "amd", gen: 7000, frameGen: false }
];
...
const GAME_DATA = [
  ...
  },
  {
    ...
    profiles: [
      ...
      { id: "very-high", name: "Very High", preset: "ultra", resolution: "4k", fps: 60, cpu: "i5-11600k", gpu: "rtx4080", ram: 32, vram: 16, storage: "ssd", rt: "off", upscaling: "native", frameGen: false },
      ...
      { id: "rt-very-high", name: "Very High Ray Tracing", preset: "ultra", resolution: "4k", fps: 60, cpu: "i5-12600k", gpu: "rtx4080", ram: 32, vram: 16, storage: "ssd", rt: "ultra", upscaling: "native", frameGen: false }
    ]
  }
];
...
const translations = {
  en: {
    ...
    excellent: "Excellent", good: "Good", fair: "Fair", limited: "Limited", full: "Full", partial: "Partial", low: "Low", mediumLabel: "Medium", highLabel: "High", balanced: "Balanced", gpuBound: "GPU bound", cpuBound: "CPU constrained", memoryBound: "VRAM pressure", storageBound: "Storage limited", noMajorLimit: "Balanced workload",
    gpuLimitDesc: "The normalized GPU ceiling is below the CPU saturation zone for this scenario.", cpuLimitDesc: "CPU saturation reduces the scaling benefit of faster GPUs in this scenario, but does not create an instant hard FPS wall.", memoryLimitDesc: "Estimated video-memory demand is above the comfortable VRAM budget.", storageLimitDesc: "The official profile expects SSD-class storage; HDD can hurt streaming and 1% lows.", balancedDesc: "The estimated CPU and GPU limits are close enough that no single component dominates.",
    ...
    howTitle: "How Reference Engine V3 works", howBody1: "FrameForge first selects the official developer profile closest to your requested resolution, preset and ray-tracing level. That profile provides a real FPS/hardware anchor.", howBody2: "V3 then compares normalized GPU and CPU performance, effective rendered pixels, preset cost, ray-tracing cost, RAM, VRAM and storage. CPU limits are modeled as progressive saturation rather than a hard FPS cutoff. The farther the extrapolation moves from the official anchor, the lower the confidence score.",
    ...
    noUpgradeNeeded: "No upgrade required", upgradeHelpful: "An upgrade can help", upgradeOptional: "Your selected target is already covered. Any upgrade here is optional headroom.", storageUpgradeReason: "The closest official profile expects SSD-class storage; NVMe mainly improves streaming and lows.", cpuUpgradeReason: "CPU saturation is the main scaling limit in this scenario.", gpuUpgradeReason: "The GPU is the first performance limit in this scenario.",
  ...
  fr: {
    ...
    excellent: "Excellent", good: "Bon", fair: "Correct", limited: "Limité", full: "Complet", partial: "Partiel", low: "Faible", mediumLabel: "Moyen", highLabel: "Élevé", balanced: "Équilibré", gpuBound: "Limité par le GPU", cpuBound: "Contraint par le CPU", memoryBound: "Pression VRAM", storageBound: "Limité par le stockage", noMajorLimit: "Charge équilibrée",
    gpuLimitDesc: "Le plafond GPU normalisé se situe sous la zone de saturation CPU pour ce scénario.", cpuLimitDesc: "La saturation CPU réduit progressivement le bénéfice d'un GPU plus rapide, sans créer un mur brutal à un nombre de FPS fixe.", memoryLimitDesc: "Le besoin estimé en mémoire vidéo dépasse la zone confortable de la VRAM.", storageLimitDesc: "Le profil officiel prévoit un stockage de classe SSD ; un HDD peut dégrader le streaming et le 1% low.", balancedDesc: "Les limites CPU et GPU estimées sont suffisamment proches pour qu'aucun composant ne domine nettement.",
    ...
    howTitle: "Comment fonctionne Reference Engine V3", howBody1: "FrameForge choisit d'abord le profil officiel du développeur le plus proche de votre résolution, de votre qualité graphique et de votre niveau de ray tracing. Ce profil fournit une véritable ancre FPS + matériel.", howBody2: "La V3 compare ensuite les performances CPU/GPU normalisées, les pixels réellement rendus, le coût du preset, du ray tracing, la RAM, la VRAM et le stockage. Les limites CPU sont modélisées comme une saturation progressive plutôt qu'une coupure nette des FPS. Plus l'extrapolation s'éloigne de l'ancre officielle, plus le score de confiance diminue.",
    ...
    officialReference: "RÉFÉRENCE OFFICIELLE", closestAnchor: "Cible développeur la plus proche", verifiedSource: "SOURCE ÉDITEUR", publisherTarget: "Cible éditeur", referenceHardware: "Matériel de référence", referenceMatch: "Correspondance", openOfficialSource: "Ouvrir la source officielle ↗", gpuRatioLabel: "Ratio GPU", cpuRatioLabel: "Ratio CPU", workloadRatioLabel: "Ratio de charge", baseRenderedLabel: "FPS de référence", relativeToReference: "vs référence officielle", lighterThanReference: "charge plus légère", heavierThanReference: "charge plus lourde", sameWorkload: "charge similaire", exactAnchor: "Ancre très proche", interpolatedAnchor: "Interpolation depuis l'ancre", extrapolatedAnchor: "Extrapolation large", refFormulaTitle: "Pourquoi ce résultat ?", refFormulaText: "FPS officiels × ratio matériel ÷ ratio de charge, puis réduction progressive à mesure que la saturation CPU augmente.", officialProfilesCount: "profils officiels"
  }
};